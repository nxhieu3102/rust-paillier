...
fn main() -> RedisResult<()> {
    ...
    let val: Option<String> = con.get("precompute_table_11")?;

    // println!("Got precompute table {:?}", val);
    let table: PrecomputeTable = match val {
        Some(val) => {
            let table: PrecomputeTable = serde_json::from_str(&val).unwrap();
            table
        },
        None => {
            let table = get_precompute_table(10);
            let encoded: Vec<u8> = bincode::serialize(&table).unwrap();
            let _: () = con.set("precompute_table_10", encoded)?;
            table
        }
    };
    // let (ek, _dk) = NKeySize2048::gen().keys();
